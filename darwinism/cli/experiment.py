...
No rendering, no display required. Usage (installed console script; ``python -m darwinism run``
and the ``run_experiment.py`` root shim are equivalent):
...
from darwinism.analysis.monitor import launch as _launch_monitor
from darwinism.config import FOX, SHEEP, make_config
...
def _load_species_brain(path: str, species_id: int, device: str = "cpu"):
    ...
    """
    from darwinism.sim.policy_brain import policy_brain_from_path
    try:
...
def main():
    ...
    if args.plot:
        from darwinism.analysis.plots import make_report
